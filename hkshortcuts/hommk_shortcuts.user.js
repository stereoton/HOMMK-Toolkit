...
try {
	...
	try {
		window.assetLoader = new AssetLoader({/*
																					 * 'MozillaLocalStorage': { 'url': 'http://pastebin.com/raw.php?i=zrfAFeBc',
																					 * 'conditions': '"undefined" == typeof window.localStorage' }, 'HkLogger': {
																					 * 'url': 'http://pastebin.com/raw.php?i=Tc4QTEkP', 'conditions': '"undefined" ==
																					 * typeof HkLogger' }, 'HkFinder': { 'url':
																					 * 'http://pastebin.com/raw.php?i=nC9wTKbb', 'conditions': '"undefined" ==
																					 * typeof HkFinder' },
																					 *//*
				 * 'HkScriptedStyles': { 'url': 'http://pastebin.com/raw.php?i=mEWtDz8K', 'conditions': '"undefined" == typeof
				 * HkStylesGeneric' }, 'MootoolsElementSelectors': { 'url': 'http://pastebin.com/raw.php?i=2G8yXznG',
				 * 'conditions': '"undefined" == typeof Element.Methods.Dom.getElementsBySelector' },
				 *//*
				 * 'MootoolsScroller': { 'url': 'http://pastebin.com/raw.php?i=W0gZUcpe' },
				 */
			'SHA256CryptoJs': {
				'url': 'http://crypto-js.googlecode.com/files/2.5.3-crypto-sha256.js'
			}
		/*
		 * , 'HkPrototypes': { 'url': 'http://pastebin.com/raw.php?i=NBX5T7pp' }
		 */
		});
	} catch(ex) {
	...
	window.hkCreateClasses = function() {
		...
		Hk.HkWindows = new Class(
		    {
			        ...
			        new Drag.Base(resizeElement, {
			            ...
			            onComplete: function(evt) {
				            ...
				            if(!this.options.hkWindow.saveWindowSize(this.options.hkWindowId, this.options.hkWindow.options)) {
					            window.console.log('[HkWindow][WARN]Resize Event Handler saveWindowSize fehlgeschlagen für '
					                + this.options.hkWindowId);
				            }
			            }
			        });
		        ...
		        getWindowSize: function getWindowSize(id, options) {
			        ...
			        window.console
			            .log('[HkWindow][DEBUG]Abgerufene Fenstergröße für  ' + win.id + ': ' + Json.toString(size));
			        return size;
		        },
		...
		if(hk.Storage.Shortcuts.isEmpty()) {
			...
			if(storageData) {
				$each(storageData,
				    function(shortcutData, shortcutName) {
